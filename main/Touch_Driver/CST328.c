...
static bool esp_lcd_touch_cst328_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x,
...
/* I2C read/write */
static esp_err_t touch_cst328_i2c_read(esp_lcd_touch_handle_t tp, uint16_t reg,
									   uint8_t *data, uint8_t len);
static esp_err_t touch_cst328_i2c_write(esp_lcd_touch_handle_t tp, uint16_t reg,
										uint8_t *data, uint8_t len);
...
esp_err_t esp_lcd_touch_new_i2c_cst328(const esp_lcd_panel_io_handle_t io,
	...
	esp_lcd_touch_handle_t esp_lcd_touch_cst328 =
		heap_caps_calloc(1, sizeof(esp_lcd_touch_t), MALLOC_CAP_DEFAULT);
	ESP_GOTO_ON_FALSE(esp_lcd_touch_cst328, ESP_ERR_NO_MEM, err, TAG,
					  "no mem for CST328 controller");
	...
	/* Save config */
	memcpy(&esp_lcd_touch_cst328->config, config,
	...
	if (esp_lcd_touch_cst328->config.int_gpio_num != GPIO_NUM_NC) {
		...
		//     esp_lcd_touch_register_interrupt_callback(esp_lcd_touch_cst328,
		//     esp_lcd_touch_cst328->config.interrupt_callback);
		// }
	}
...
err:
	if (ret != ESP_OK) {
		ESP_LOGE(TAG,
				 "Error (0x%x)! Touch controller CST328 initialization failed!",
				 ret);
...
static esp_err_t esp_lcd_touch_cst328_read_data(esp_lcd_touch_handle_t tp) {
	...
	err =
		touch_cst328_i2c_read(tp, ESP_LCD_TOUCH_CST328_READ_Number_REG, buf, 1);
	ESP_RETURN_ON_ERROR(err, TAG, "I2C read error!");
	// touch_cst328_i2c_write(tp, ESP_LCD_TOUCH_CST328_READ_XY_REG, &Over, 1);
	/* Any touch data? */
	if ((buf[0] & 0x0F) == 0x00) {
		touch_cst328_i2c_write(tp, ESP_LCD_TOUCH_CST328_READ_Number_REG, &clear,
							   1); // No touch data
	} else {
		/* Count of touched points */
		touch_cnt = buf[0] & 0x0F;
		if (touch_cnt > 5 || touch_cnt == 0) {
			touch_cst328_i2c_write(tp, ESP_LCD_TOUCH_CST328_READ_Number_REG,
								   &clear, 1);
			return ESP_OK;
		}
		...
		/* Read all points */
		// err = touch_cst328_i2c_read(tp, ESP_LCD_TOUCH_CST328_READ_XY_REG,
		// &buf[1], touch_cnt * 5 + 2);
		err = touch_cst328_i2c_read(tp, ESP_LCD_TOUCH_CST328_READ_XY_REG,
									&buf[1], 27);
		ESP_RETURN_ON_ERROR(err, TAG, "I2C read error!");
		// touch_cst328_i2c_write(tp, ESP_LCD_TOUCH_CST328_READ_XY_REG, &Over,
		// 1);

		/* Clear all */
		err = touch_cst328_i2c_write(tp, ESP_LCD_TOUCH_CST328_READ_Number_REG,
									 &clear, 1);
		ESP_RETURN_ON_ERROR(err, TAG, "I2C read error!");
		...
		for (i = 0; i < touch_cnt; i++) {
			if (i > 0)
				num = 2;
			tp->data.coords[i].x =
				(uint16_t)(((uint16_t)buf[(i * 5) + 2 + num] << 4) +
						   ((buf[(i * 5) + 4 + num] & 0xF0) >> 4));
			tp->data.coords[i].y =
				(uint16_t)(((uint16_t)buf[(i * 5) + 3 + num] << 4) +
						   (buf[(i * 5) + 4 + num] & 0x0F));
...
static void touch_cst328_read_cfg(esp_lcd_touch_handle_t tp) {
	...
	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_BOOT_TIME,
						  (uint8_t *)&buf[0], 4);
	ESP_LOGI(TAG, "TouchPad_ID:0x%02x,0x%02x,0x%02x,0x%02x", buf[0], buf[1],
			 buf[2], buf[3]);

	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_RES_X, (uint8_t *)&buf[0],
						  1);
	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_RES_X + 1,
						  (uint8_t *)&buf[1], 1);
	ESP_LOGI(TAG, "TouchPad_X_MAX:%d", buf[1] * 256 + buf[0]);
	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_RES_Y, (uint8_t *)&buf[2],
						  1);
	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_RES_Y + 1,
						  (uint8_t *)&buf[3], 1);
	ESP_LOGI(TAG, "TouchPad_Y_MAX:%d", buf[3] * 256 + buf[2]);

	touch_cst328_i2c_read(tp, CST328_REG_DEBUG_INFO_TP_NTX, buf, 24);
	ESP_LOGI(TAG, "D1F4:0x%02x,0x%02x,0x%02x,0x%02x", buf[0], buf[1], buf[2],
			 buf[3]);
	ESP_LOGI(TAG, "D1F8:0x%02x,0x%02x,0x%02x,0x%02x", buf[4], buf[5], buf[6],
			 buf[7]);
	ESP_LOGI(TAG, "D1FC:0x%02x,0x%02x,0x%02x,0x%02x", buf[8], buf[9], buf[10],
			 buf[11]);
	ESP_LOGI(TAG, "D200:0x%02x,0x%02x,0x%02x,0x%02x", buf[12], buf[13], buf[14],
			 buf[15]);
	ESP_LOGI(TAG, "D204:0x%02x,0x%02x,0x%02x,0x%02x", buf[16], buf[17], buf[18],
			 buf[19]);
	ESP_LOGI(TAG, "D208:0x%02x,0x%02x,0x%02x,0x%02x", buf[20], buf[21], buf[22],
			 buf[23]);
...
esp_err_t Touch_I2C_Init(void) {
	...
	if (i2c_new_master_bus(&conf, &i2c_bus_handle) != ESP_OK) {
...
void TOUCH_Init(void) {
	...
	esp_lcd_panel_io_i2c_config_t tp_io_config =
		ESP_LCD_TOUCH_IO_I2C_CST328_CONFIG();
