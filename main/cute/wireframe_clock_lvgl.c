...
static void get_current_time(uint8_t *h24, uint8_t *h12, uint8_t *m,
							 uint8_t *s) {
...
static void clock_timer_cb(lv_timer_t *timer) {
	...
	hour_pts[0].x = CLOCK_CENTER_X;
	hour_pts[0].y = CLOCK_CENTER_Y;
	polar_point(hour_angle, HAND_LEN_HOUR, &hour_pts[1]);

	min_pts[0].x = CLOCK_CENTER_X;
	min_pts[0].y = CLOCK_CENTER_Y;
	polar_point(min_angle, HAND_LEN_MIN, &min_pts[1]);

	sec_pts[0].x = CLOCK_CENTER_X;
	sec_pts[0].y = CLOCK_CENTER_Y;
...
static void eye_blink_once(lv_obj_t *eye) {
	...
	lv_anim_set_playback_time(&a,
							  EYE_BLINK_ANIM_MS); /* revient automatiquement */
	lv_anim_start(&a);
}
...
void wireframe_clock_create(lv_obj_t *parent) {
	...
	eye_blink_timer =
		lv_timer_create(eye_blink_timer_cb, EYE_BLINK_PERIOD_MS, NULL);
