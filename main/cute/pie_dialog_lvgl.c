...
static int32_t pie_hit_test(int32_t dx, int32_t dy) {
	...
	if (dist_sq > (int64_t)PIE_RADIUS * PIE_RADIUS)
		return -1;

	if (dx >= 0 && dy < 0)
		return 0; /* haut-droit  -> Section 1 */
	if (dx >= 0 && dy >= 0)
		return 1; /* bas-droit   -> Section 2 */
	if (dx < 0 && dy >= 0)
		return 2; /* bas-gauche  -> Section 3 */
...
static void pie_wheel_build_spokes(lv_obj_t *parent) {
	...
	spoke_right[0].x = PIE_RADIUS;
	spoke_right[0].y = PIE_RADIUS;
	spoke_right[1].x = PIE_RADIUS * 2 - 2;
	spoke_right[1].y = PIE_RADIUS;

	spoke_down[0].x = PIE_RADIUS;
	spoke_down[0].y = PIE_RADIUS;
	spoke_down[1].x = PIE_RADIUS;
	spoke_down[1].y = PIE_RADIUS * 2 - 2;

	spoke_left[0].x = PIE_RADIUS;
	spoke_left[0].y = PIE_RADIUS;
	spoke_left[1].x = 2;
	spoke_left[1].y = PIE_RADIUS;

	spoke_up[0].x = PIE_RADIUS;
	spoke_up[0].y = PIE_RADIUS;
	spoke_up[1].x = PIE_RADIUS;
	spoke_up[1].y = 2;
...
static void pie_wheel_build(lv_obj_t *wheel_container) {
	...
	lv_obj_add_event_cb(pie_hit_overlay, pie_overlay_event_cb, LV_EVENT_PRESSED,
						NULL);
	lv_obj_add_event_cb(pie_hit_overlay, pie_overlay_event_cb,
						LV_EVENT_PRESSING, NULL);
	lv_obj_add_event_cb(pie_hit_overlay, pie_overlay_event_cb,
						LV_EVENT_RELEASED, NULL);
	lv_obj_add_event_cb(pie_hit_overlay, pie_overlay_event_cb,
						LV_EVENT_PRESS_LOST, NULL);
	lv_obj_add_event_cb(pie_hit_overlay, pie_overlay_event_cb, LV_EVENT_CLICKED,
						NULL);
}
...
static void pie_dialog_modal_bg_event_cb(lv_event_t *e) {
	if (lv_event_get_code(e) != LV_EVENT_CLICKED)
		return;
	if (lv_event_get_target(e) != pie_dialog_modal_bg)
		return;
	pie_dialog_close();
}
...
void pie_dialog_open(lv_obj_t *parent_scr) {
	...
	lv_obj_add_event_cb(pie_dialog_modal_bg, pie_dialog_modal_bg_event_cb,
	...
	lv_obj_add_flag(pie_dialog_panel,
					LV_OBJ_FLAG_CLICKABLE); /* absorbe le clic, ne ferme pas */
	...
	lv_obj_add_event_cb(close_btn, pie_dialog_close_btn_event_cb,
						LV_EVENT_CLICKED, NULL);
