...
void SD_Init(void) {
	...
	// formatted in case when mounting fails.  false true
	...
	// Enable internal pullups on enabled pins. The internal pullups are
	// insufficient however, please make sure 10k external pullups are connected
	// on the bus. This is for debug / example purpose only.
...
FILE *Open_File(const char *file_path) {
	...
	if (fp == NULL) {
		ESP_LOGE(SD_TAG, "Failed to open file %s. Error: %s", file_path,
				 strerror(errno));
	} else
...
uint16_t Folder_retrieval(const char *directory, const char *fileExtension,
	...
	while ((entry = readdir(dir)) != NULL && fileCount < maxFiles) {
		...
		if (dot != NULL && dot != entry->d_name) {
			...
			if (strcasecmp(dot, fileExtension) == 0) {
				...
				strncpy(File_Name[fileCount], entry->d_name,
						MAX_FILE_NAME_SIZE - 1);
				File_Name[fileCount][MAX_FILE_NAME_SIZE - 1] = '\0';

				char filePath[MAX_PATH_SIZE];
				snprintf(filePath, MAX_PATH_SIZE, "%s/%s", directory,
						 entry->d_name);
			...
			// If the extension name is not found, you can output debugging
			// information printf("No extension found for file: %s\r\n",
			// entry->d_name);
		}
	}
	...
	if (fileCount > 0) {
		ESP_LOGI(SD_TAG, "Retrieved %d files with extension '%s'", fileCount,
				 fileExtension); //
	} else {
		ESP_LOGW(SD_TAG, "No files with extension '%s' found in directory: %s",
